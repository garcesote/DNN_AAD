...
#         train_dnn(model=model, dataset=dataset, data_path=data_path, key=date, mdl_save_path=mdl_save_path,
#                   metrics_save_path=metrics_save_path, max_epoch=200, filt_path=paths[dataset+'_filt_path'])

# # EVALUATE DNN
# for model in models:
#     for dataset in datasets:
#         data_path = paths[dataset+'_path']
#         eval_dnn(model, dataset, data_path, results_save_path, mdl_save_path, date , filt_path=paths[dataset+'_filt_path'])

# # TRAINIG RIDGE
# for dataset in datasets:
#     data_path = paths[dataset+'_path']
#     train_ridge(dataset, data_path, mdl_save_path=mdl_save_path, key=date, original=True, filt_path=paths[dataset+'_filt_path'])
#     eval_ridge(dataset, data_path, mdl_save_path, key= date, dst_save_path= results_save_path, original=True, filt_path=paths[dataset+'_filt_path'])
#     train_ridge(dataset, data_path, mdl_save_path=mdl_save_path, key=date, original=False, filt_path=paths[dataset+'_filt_path'])
#     eval_ridge(dataset, data_path, mdl_save_path, key= date, dst_save_path= results_save_path, original=False, filt_path=paths[dataset+'_filt_path']) 
...
for dataset in datasets:
    for model in models:
        for win in window_lenghts:
            decode_attention(model, dataset, win, paths[dataset+'_path'], mdl_save_path, accuracies_save_path, key=date, filt_path=paths[dataset+'_filt_path'])
